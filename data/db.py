...
async def add_favourite(user_id, name):
    cs.execute(f"SELECT * FROM favourites_data WHERE favourites_uid = {user_id} and favourites_name = '{name}'")
    if cs.fetchall() != []:
        return False
...
async def delete_favourite(user_id, name):
    cs.execute(f"DELETE FROM favourites_data WHERE favourites_uid = {user_id} and favourites_name = '{name}'")
    sql.commit()
